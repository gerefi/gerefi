...
void receiveCANwbo() 
{
  if(configPage2.canWBO == CAN_WBO_RUSEFI) //RusEFI CAN Wideband supported: https://github.com/mck1117/wideband
  {
    ...
    outMsg.len = 2;
    outMsg.buf[0] = currentStatus.battery10; // We don't do any conversion since factor is 0.1 and gerefi value is x10
...
void can_Command(void)
{
  ...
  {
    // The address is the gerefi specific ecu canbus address 
  ...
  {
    // The address is only the gerefi specific ecu canbus address    
    if (inMsg.buf[1] == 0x09)
    {
      ...
      {
      //code 20: send 20 ascii characters with ECU name , "ECU -gerefiXXXXXX" , change the XXXXXX ONLY as required.  
      }
    }
  }
} 
